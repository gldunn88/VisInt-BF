...
class HudRenderer():
    ...
    def __init__(self, interpreter: BFInterpreter, typeface: str = "freesansbold.ttf", point_size: int = 32):
        ...
        self.hertz_text: pg.Surface = None
        self.step_text: pg.Surface = None
        self.state_text: pg.Surface = None

    def renderHud(self, screen: pg.Surface, hud_rect: pg.Rect, hertz: int):
        if self.last_hertz != hertz:
            self.hertz_text = self.font.render(f"CPU: {hertz}Hz", True, rc.CLR_WHITE, rc.CLR_BLACK)
            self.last_hertz = hertz

        if self.interpreter.state != self.last_interpreter_state:
            self.state_text = self.font.render(f"State: {self.interpreter.state._name_}", True, rc.CLR_WHITE, rc.CLR_BLACK) # noqa 
            self.last_interpreter_state = self.interpreter.state

        if self.last_step_count != self.interpreter.step_count:
            self.step_text = self.font.render(f"Step Count: {self.interpreter.step_count}", True, rc.CLR_WHITE, rc.CLR_BLACK) # noqa
            self.last_step_count = self.interpreter.step_count

        hertz_rect = self.hertz_text.get_rect()
        state_rect = self.state_text.get_rect()
        step_rect = self.step_text.get_rect()
        ...
        screen.blit(self.hertz_text, hertz_rect)
        screen.blit(self.state_text, state_rect)
        screen.blit(self.step_text, step_rect)
